    ...
    try:
        ...
        with transaction.atomic():
            for user_item in users_data:
                ...
                # logger.debug(f"{action} user: {user.username}") # debug, щоб не засмічувати логи
...
def fetch_posts():
    ...
    try:
        ...
        # values_list не підходить, бо нам потрібні самі об'єкти для ForeignKey
        existing_users = {
            user.external_id: user 
            for user in SocialUser.objects.all()
        }
        ...
        logger.info(f"Posts processing finished. Saved/Updated: {created_count}. Skipped (no author): {skipped_count}")
        ...
        try:
            ...
            with transaction.atomic():
                for comm_item in comments_data:
                    # author_email може не бути в API, тому використовуємо .get
                    # У DummyJSON це user.username, але ми домовились про author_email в моделі.
                    # Для спрощення запишемо туди username користувача з API коментаря.
                    ...
                    Comment.objects.update_or_create(
                        ...
                        defaults={
                            "body": comm_item["body"],
                            "author_email": f"{username}@example.com", # Імітація email
                            "post": post  # Прив'язуємо до нашого об'єкта Post
                        }
                    )
                    total_created += 1
